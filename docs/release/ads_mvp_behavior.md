...
This document defines the release-time ad behavior for the MVP. It is a contract note for QA, store readiness, and privacy review, not an implementation file.

## Allowed placements

- Passive banner-style ads are allowed only in non-sensitive, browse-style areas of the app.
- In the current MVP scope, the intended allowed placements are the Home feed, Play list, and Sounds list.
- Ads should only appear when the user is on a stable screen state and the entitlement allows free-user ads.
...
- Banner: allowed in the passive slots above.
- Interstitial: not allowed in the MVP.
- Rewarded: not allowed in the MVP.

## Free vs Premium behavior

- Free users may see passive banner ads in the allowed browse areas.
...
- Data Safety, privacy policy, and store listing copy should describe the same MVP contract.
