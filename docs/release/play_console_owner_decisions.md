...
| Privacy policy URL handoff | Keep the live policy URL aligned with the shipped behavior and the public SSOT at `https://nurtly.graylion.pl/privacy` | The URL should point to the final policy, not a placeholder, and it should not drift from the live mirror. | Required for public release and expected for closed-testing readiness planning. | A placeholder URL breaks trust and can block publication. | Yes | Yes | Yes |
...
- Privacy policy: keep the live public URL and mirrors aligned with the shipped build.
