...
| Privacy policy URL handoff | Keep the live policy URL aligned with the shipped behavior and the public SSOT at `https://nurtly.graylion.pl/privacy` | The URL should point to the final policy, not a placeholder, and it should not drift from the live public source. | Required for public release and expected for closed-testing readiness planning. | A placeholder URL breaks trust and can block publication. | Yes | Yes | Yes |
...
- Privacy policy: keep the live public URL and public policy docs aligned with the shipped build.
- Privacy policy publication: use the handoff in `docs/legal/privacy_policy_publication_handoff.md` and keep the live URL at `https://nurtly.graylion.pl/privacy`.
- Data Safety: keep the MVP disclosure limited to local-only Journal, billing, passive ads, and no product analytics provider.
- Advertising ID: treat as owner-confirmed and do not assume it is required; final release-candidate permission evidence remains pending in issue `#185`.
...
- The approved public privacy URL is live at https://nurtly.graylion.pl/privacy; keep Play Console aligned with the live SSOT and use the privacy publication handoff as the source of record.
- Public privacy publication and anonymous reachability are resolved, but BLK-005 remains open for Play Console and release compliance work.
